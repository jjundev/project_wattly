...
화면 하단 `CONTROL STRIP`(시나리오/배터리/테마/모드/히어로)과 바깥 페이지 헤더는 **데모 하니스**이지 앱 기능이 아니다. 설정 창에 레이아웃 모드 토글이 없으므로 **A/B/C는 디자인 후보 비교용**, 출시는 **모드 A**만(→ [03 보류 결정 #23](#보류-결정-사용자-확인-필요)).
...
| 팝오버 레이아웃 3종 | 1종 암시 | A/B/C | **A만 출시** → [02](02-popover-mode-a-cards.md) |
...
**빌드 순서:** `01 → 02·03`(셸) → `04`(진짜 MVP) → `05·06·07·08·11·12` 병렬 → `09·10` → `14 → 13` → `15·16` → `17`. `18`은 전 구간 병렬.
...
**Confident #1–20:** 1 진실의소스, 2 네이티브SwiftUI, 3 모드A만, 4 드래그재정렬, 5 테마3종, 6 임곗값+색상, 7 배터리온도, 8 메뉴바온도텍스트, 9 배터리부호W, 10 코어 perflevel, 11 자체소비푸터, 12 데이터소스맵, 13 상태모델, 14 스파크라인60s, 15 단일seam, 16 설정scene, 17 적응형주기, 18 패키징, 19 시각토큰, 20 메뉴바아이템.
...
| 23 | 레이아웃 **모드 A만** 출시? | **예** (B/C 미구현) | [02](02-popover-mode-a-cards.md) |
...
가장 답이 급한 건 **#21·#22·#23** — 범위·일정이 여기서 갈린다.
