...
- **무권한(Zero-Privilege) 텔레메트리 & 도우미 데몬 진단**: 읽기 전용 텔레메트리는 `IOReport`, AppleSMC 읽기 키, Mach 커널 API, IOHID를 통해 100% 표준 사용자 공간(User-space)에서 동작합니다. 일반 모니터링 시 root 권한이나 백그라운드 데몬이 불필요하며, 시스템 쓰기(팬·충전 제어)를 담당하는 도우미 데몬은 실시간 상태 진단(Health Diagnostics)과 설정 내 원클릭 복구(One-Click Reinstall/Repair)를 지원합니다.
- **배터리 수명 관리 슈트 (Battery Health Suite)**: BMS 오차를 보정하는 6단계 자동 배터리 캘리브레이션(Battery Calibration Suite), 80%~95% 맞춤형 충전 상한 제한(Charge Limit) 및 동적 충전 완료 시간(Target ETA, 85%·90%), 12시간 자동 만료 안전장치가 적용된 일회성 완충(One-Time Top-Up), 실시간 소모 전력(-W) 기반 자동 및 수동 강제 방전(Discharge, 50%~99%), 요일/시간대별 충전 스케줄링 지원.
...
- **밀리와트(mW) 단위 SoC 전력 분석 & 실시간 프로세서 식별**: `machdep.cpu.brand_string` 커널 쿼리를 통한 실시간 Apple Silicon 칩셋 모델 식별 및 비공개 `IOReport` Energy Model 구독을 통한 CPU, GPU, Apple Neural Engine (ANE) 실시간 전력 추적과 4초 EMA(지수 이동 평균) 필터링.
...
- **동적 목표 충전 완료 시간 (Target ETA)**: 기존의 단순 100% 완충 기준 잔여 시간 대신, 현재 활성화된 충전 상한선(예: 85%, 90%)에 도달하는 데 필요한 에너지를 실시간 충전 전력(W)과 잔여 용량(Wh)을 바탕으로 정밀 계산하여 표시합니다 (예: `"85%까지 약 25분 남음"` 또는 `"90%까지 약 40분 남음"`). 일회성 완충(Top-Up)을 시작하면 자동으로 `"완충까지 약 1시간 남음"`으로 매끄럽게 전환됩니다.
...
- **12시간 자동 만료 안전장치 및 알림**: 100% 완충 후 전원 어댑터를 분리하면 기존에 설정해 둔 충전 한도(예: 80%)로 자동 복귀합니다. 어댑터를 계속 꽂아 두더라도 100% 도달 후 12시간이 지나면 헬퍼가 고전압 상태 방치를 방지하기 위해 스스로 Top-Up을 해제하고 시스템 알림을 전송하므로, 켜 두고 잊어도 배터리가 100%에 무기한 머물지 않습니다.
- **수동 방전과의 상호 배제 (Mutual Exclusion)**: 충전과 방전 로직의 충돌을 방지하기 위해, 일회성 완충이 활성화되어 있는 동안에는 수동 방전 제어가 상호 배제 처리됩니다.
...
- **자동 방전 (Auto Discharge) vs 수동 방전 (Manual Discharge)**:
  - **자동 방전 (Auto Discharge)**: 충전 한도를 현재 잔량보다 낮게 조정하면(예: 현재 90% 상태에서 한도를 80%로 변경) 추가 조작 없이 자동으로 한도까지 방전합니다.
  - **수동 방전 (Manual Discharge)**: 50%~99% 범위의 슬라이더로 목표 잔량을 직접 지정하고 실시간 소모 전력(-W) 및 예상 완료 시간을 확인하며 즉시 방전을 실행할 수 있습니다 (잔량이 100%를 초과할 수 없어 조작 실효성이 없는 100% 대신 상한선은 99%로 제한됩니다).
- **실시간 소모 전력(-W) 기반 동적 방전 ETA**: 기존의 고정 추정치 대신, 실시간 시스템 순방전 전력(음수 와트, -W)과 배터리 잔여 Wh를 바탕으로 남은 방전 시간을 정밀 계산합니다. 방전 시작 직후의 급격한 전류 변동 노이즈를 걸러내는 EMA 웜업 윈도우(Warm-up Window, 10초)를 거친 후 신뢰할 수 있는 예상 완료 시간을 제공합니다.
- **하드웨어 역량 게이팅 (CHIE Hardware Gating)**: 하드웨어 SMC에서 `CHIE` 레지스터 제어를 공식 지원하는 Apple Silicon 기기에서만 방전 인터페이스가 안전하게 활성화됩니다.

### 6. 배터리 캘리브레이션 모드 (Battery Calibration Suite)
- **BMS 오차 누적 및 갑작스러운 꺼짐 방지**: 상시 AC 전원 연결 및 충전 상한 제한을 장기간 유지할 경우 배터리 관리 시스템(BMS)의 잔량 계측(Gas Gauge) 기준점이 점진적으로 틀어져 잔량이 남아 있음에도 시스템이 갑자기 꺼지는 현상이 발생할 수 있습니다. 캘리브레이션 슈트는 BMS의 상·하한 기준점을 재학습시켜 잔량 표시 오차를 완벽히 교정합니다.
- **6단계 전자동 사이클**:
  1. **시작 준비 (Preflight)**: 도우미 데몬 연결, 어댑터 연결 상태, 하드웨어 역량, 발열 상태 및 macOS "최적화된 배터리 충전" 비활성화 등 필수 전제 조건을 사전 점검합니다.
  2. **100% 완충 (Full Charge)**: 배터리를 100%까지 완전 충전합니다.
  3. **1차 쿨다운 (Cooldown 1)**: 완충 상태에서 셀 전압 및 화학 반응이 안정화되도록 대기합니다.
  4. **저잔량 강제 방전 (Forced Discharge to 15%)**: 시스템 유휴 잠자기를 방지하는 `SleepAssertion`(`IOPMAssertionCreateWithName`)을 유지한 채 15% 하한선까지 안전하게 배터리를 강제 방전합니다.
  5. **2차 쿨다운 (Cooldown 2)**: 저잔량 도달 후 10분간 화학 평형을 안착시키는 저잔량 안정화(Soak Low)를 진행합니다.
  6. **100% 재충전 및 복원 (Full Re-charge & Restore)**: 배터리를 다시 100%까지 완전 충전하여 BMS의 풀 스케일 기준점을 갱신하고, 60분 최종 안정화(Soak Final)를 거쳐 기존 충전 한도 및 사용자 설정으로 안전하게 복귀합니다.
- **전용 진단 UI 카드**: 설정 › 배터리 화면에 진행률 표시줄(Progress Bar), 현재 단계 및 잔여 시간 분해 표시, 언제든 안전하게 중단할 수 있는 안전 중단(Safe Abort) 기능, 90일 또는 40사이클 주기 쿨다운 안내 및 과거 캘리브레이션 이력 팝오버를 제공합니다.
...
2. **격리된 도우미 데몬 (`WattlyHelperDaemon` / `dev.jjundev.WattlyFanDaemon`)**:
   ...
   - **실시간 상태 진단 및 복구 메커니즘 (Health Diagnostics & Recovery)**: XPC 핑 응답, 바이너리 버전 일치 여부, launchd 등록 상태, root:wheel 소유권 및 파일 권한을 실시간으로 감시합니다. 데몬 응답 지연이나 권한 손상 감지 시 설정 화면을 통해 즉각적인 원클릭 복구 경로를 제공합니다.
...
- **시스템 도우미 상태 진단 및 원클릭 복구 (Helper Daemon Diagnostics & Repair)**: '일반' 섹션에서 도우미 데몬의 실행 상태, 버전 일치 여부, XPC 통신 건전성을 직관적인 상태 뱃지로 확인할 수 있으며, 데몬 오류 발생 시 클릭 한 번으로 안전하게 재등록 및 수리(`One-Click Reinstall/Repair`)할 수 있습니다 (다른 계정 설치 시 소유권 이전 지원).
