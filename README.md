...
- **6단계 전자동 사이클**:
  ...
  3. **20% 강제 방전 (Discharge to 20%)**: 시스템 유휴 잠자기를 방지하는 `SleepAssertion`(`IOPMAssertionCreateWithName`)을 유지한 채 20% 하한선까지 안전하게 배터리를 강제 방전합니다.
  4. **저잔량 안정화 (Low-Charge Soak)**: 20% 저잔량 도달 후 10분간 전압 및 화학 평형을 안착시키는 안정화 과정을 진행합니다.
  5. **100% 재충전 (Full Re-charge)**: 배터리를 다시 100%까지 재충전하여 BMS의 전체 용량(Full Scale) 기준점을 갱신합니다.
  6. **최종 안정화 및 복원 (Final Soak & Restore)**: 60분간의 최종 안정화(Soak Final)를 완료한 후 기존 충전 한도 및 사용자 설정으로 안전하게 복귀합니다.
