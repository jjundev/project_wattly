...
enum MenuBarText {
    ...
    static func assemble(items: [MenuBarItem], states: [CardKind: MetricState], locale: Locale = Locale(identifier: "ko")) -> String? {
        guard !items.isEmpty else { return nil }
        let parts = items.map { formatItem($0, states: states, locale: locale) }
        return parts.joined(separator: "  ·  ")
    }

    /// Formats a single `MenuBarItem` using the appropriate card state from `states`.
    static func formatItem(_ item: MenuBarItem, states: [CardKind: MetricState], locale: Locale = Locale(identifier: "ko")) -> String {
        let state = states[item.requiredCard] ?? .loading
        switch item {
        case .card(let kind):
            return part(kind, state, locale: locale)
        case .coreClock(let prefix):
            return coreClockPart(prefix, state, locale: locale)
        case .memPressure:
            return memPressurePart(state, locale: locale)
        case .batteryTemp:
            return batteryTempPart(state, locale: locale)
        }
    }
    ...
    static func assemble(selected: Set<CardKind>, states: [CardKind: MetricState], locale: Locale = Locale(identifier: "ko")) -> String? {
        let items = order.filter(selected.contains).map { MenuBarItem.card($0) }
        return assemble(items: items, states: states, locale: locale)
    }
    ...
    /// are always safe.
    static func part(_ card: CardKind, _ state: MetricState, locale: Locale = Locale(identifier: "ko")) -> String {
        guard case .value(let sample) = state else { return "\(longLabel(card, locale: locale)) —" }
        switch (card, sample) {
        ...
        case (.cpuTemp, .temperature(let s)): return tempPart("CPU", longLabel(card, locale: locale), s.cpu)
        case (.gpuTemp, .temperature(let s)): return tempPart("GPU", longLabel(card, locale: locale), s.gpu)
        case (.fan, .fan(let s)):
            return averageRPM(s.fans).map { "\(String(localized: "팬", locale: locale)) \(Int($0.rounded())) RPM" } ?? "\(longLabel(card, locale: locale)) —"
        default:                              return "\(longLabel(card, locale: locale)) —"   // state/sample mismatch
        }
    }
    ...
    private static func longLabel(_ card: CardKind, locale: Locale = Locale(identifier: "ko")) -> String {
        switch card {
        case .cpu: "CPU"
        case .gpu: "GPU"
        case .power: String(localized: "전력", locale: locale)
        case .mem: String(localized: "메모리", locale: locale)
        case .cpuTemp: String(localized: "CPU 온도", locale: locale)
        case .gpuTemp: String(localized: "GPU 온도", locale: locale)
        case .fan: String(localized: "팬", locale: locale)
        case .battery: String(localized: "배터리", locale: locale)
        }
    }
    ...
    static func batteryTempPart(_ state: MetricState, locale: Locale = Locale(identifier: "ko")) -> String {
        guard case .value(.battery(let s)) = state, let c = s.temperatureCelsius else {
            return "\(String(localized: "배터리 온도", locale: locale)) —"
        }
        return "\(String(localized: "배터리", locale: locale)) \(Int(c.rounded()))°C"
    }
    ...
    static func memPressurePart(_ state: MetricState, locale: Locale = Locale(identifier: "ko")) -> String {
        guard case .value(.memory(let s)) = state, let p = s.pressurePercent else {
            return "\(String(localized: "압력", locale: locale)) —"
        }
        return "\(String(localized: "압력", locale: locale)) \(p)%"
    }
    ...
    static func coreClockPart(_ prefix: String, _ state: MetricState, locale: Locale = Locale(identifier: "ko")) -> String {
        let cold = "\(prefix) \(String(localized: "코어 클럭", locale: locale)) —"
