...
- 팝오버 눈 확인(Step 1): **통과(사용자 직접 확인, 2026-09-17).** 브랜치 Debug 빌드(`5e7b3e1`)를 실행해 `yes` 4개 부하를 두 차례 걸었다 풀며 프로세서 전력 카드를 확인 — 확인 항목(헤드라인·CPU 서브값 매초 갱신, 부하에 다음 폴부터 반응, 스파이크 없음, ANE 0.0 W 유지)에 대해 사용자가 "통과"로 보고. 카드에 표시된 구체 수치는 기록되지 않았다.
