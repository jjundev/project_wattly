...
- **Second behaviour change:** two genuinely different programs that ship the same bundle id (Xcode and Xcode-beta both declare `com.apple.dt.Xcode`; some template-built Electron apps reuse a boilerplate id) now merge into one row, labelled from whichever member is largest. Accepted — Activity Monitor's own grouping is no better — but it is the neighbouring case to the intended "two copies of one app" win.
- **Third behaviour change:** a memory row for an unreadable-path (`PID n`) group used to carry `iconPath: "PID 123"` and draw `NSWorkspace`'s generic icon; it now carries `nil` and draws the card's faint placeholder, matching the power card. An improvement, but a user-visible pixel change beyond the intended fix.
