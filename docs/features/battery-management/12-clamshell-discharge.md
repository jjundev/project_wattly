...
- **이 만료 래치는 메모리 전용이라 절대적이지 않다**: `clamshellExpiredForCurrentDischarge`는 디스크에 저장되지 않으므로, 12시간을 넘는 방전 도중 데몬이 재시작되면 래치가 지워진다. 게다가 `restore`는 시작할 때 고아 마커를 무조건 정리하므로, 재시작 이후의 다음 reconcile은 (세 조건이 다시 모두 참이면) 또 다른 12시간짜리 잠자기 차단을 새로 시작할 수 있다. 이 재개는 방향이 안전하다 — `restore`가 먼저 플래그를 물리적으로 내리고, 재개도 여전히 세 조건(옵트인·외장 디스플레이·진행 중 방전)을 모두 요구한다 — 하지만 "한 번 방전당 최대 12시간"이 곧 "재부팅 사이 최대 12시간"을 뜻하지는 않는다는 점은 분명히 해 둔다.
...
|---|---|---|---|---|
| 1 | **실제 뚜껑 닫힘 (핵심 검증)** | 외장 디스플레이 연결 + 어댑터 연결 + 설정에서 "덮개를 닫아도 방전 계속" ON. 수동 방전을 시작한 뒤, 노트북 뚜껑을 실제로 닫는다. `AppleClamshellCausesSleep`은 이 기능과 무관하게 항상 `Yes`로 남으므로 검증에 쓸 수 없다 — 아래 관찰만이 유일한 증거다. | 뚜껑을 닫은 채로: 외장 화면이 계속 켜져 있다, `pmset -g`의 `SleepDisabled`가 1이다, 몇 분 뒤 SoC가 실제로 하락해 있다(정지해 있지 않다) |**통과** 2026-09-08. 뚜껑 닫고 10.4분, 20초 간격 32개 샘플 누락 0회(=깨어 있었음), `SleepDisabled` 전 구간 1, SoC 70→69, 모니터 계속 점등 |
| 2 | 어댑터 분리 | 1번 상태에서 전원 어댑터를 뽑는다 | 5초 내 방전이 취소되고 `SleepDisabled`가 0으로 내려간다, 이후 Mac이 정상적으로 잠자기에 들어간다(뚜껑이 닫혀 있으면 즉시, 열려 있으면 idle 타이머 경과 후) |**통과** 2026-09-08. 분리 3~10초 내 `SleepDisabled` 1→0 |
| 3 | 외장 모니터 분리 | 1번 상태에서 외장 모니터 케이블을 뽑는다 | 앱이 `allowed=false`를 보내 `SleepDisabled`가 0으로 내려가고, Mac이 잠자기에 들어간다 |**통과** 2026-09-08. 분리 20초 내 1→0, `ExternalConnected=No` 유지·SoC 계속 하락 = 방전 생존 |
| 4 | 데몬 강제 종료 | 방전 진행 중 `sudo kill -9 <daemon pid>` | launchd 재기동 직후 `SleepDisabled`가 0으로 정리된다(고아 마커 회수). **방전은 재개되지 않는다** — `persistPolicy`가 `manualDischargeActive`를 `false`로 저장하기 때문이며 설계된 동작이다. 방전이 없으므로 `SleepDisabled`는 계속 0으로 남는 것이 정답이고, 다시 1이 되려면 사용자가 방전을 다시 시작해야 한다 |**통과** 2026-09-08. 재기동 직후 1→0(고아 마커 회수). 방전은 설계대로 재개되지 않음 |
| 5 | 목표 도달 | 수동 방전이 목표 SoC에 도달할 때까지 기다린다(또는 낮은 목표로 설정) | `SleepDisabled`가 0으로 내려가고, 진행 배너의 "잠자기 차단 중" 줄이 사라진다 |미검증. 방전 종료 시 해제는 2·3·6번에서 다른 종료 사유로 3회 관측됨 |
| 6 | 사용자가 직접 켜둔 값은 건드리지 않는다 | 방전을 시작하기 **전에** `sudo pmset -a disablesleep 1`을 미리 실행해 둔 뒤, 옵션 ON 상태로 방전을 시작했다가 종료까지 진행 | 시작·진행·종료 전 과정에서 Wattly가 값을 바꾸지 않는다(계속 1로 유지) |**통과** 2026-09-08. 방전 시작·진행·종료 전 구간 1 유지 — 사용자 설정을 가로채지 않음 |
| 7 | 앱 삭제 흐름 | 방전이 진행 중인 상태에서 공식 삭제 흐름(헬퍼 제거 포함)을 실행한다 | 삭제 완료 후 `pmset -g`의 `SleepDisabled`가 0 |미검증(헬퍼 재설치 필요) |
| 8 | 외장 디스플레이 없이 시작 | 옵션 ON, 외장 디스플레이는 연결하지 않은 채 수동 방전 시작 | `clamshellDischargeAllowed`가 false로 계산되어 `SleepDisabled`는 계속 0 |**통과** 2026-09-08. 옵트인 켠 상태에서도 `SleepDisabled` 0 유지 |
| 9 | 캘리브레이션 방전 단계 | 옵션 ON, 외장 디스플레이 연결 상태에서 캘리브레이션 모드를 시작해 방전 단계(`dischargeToFloor`)까지 진행 | 방전 단계에서만 `SleepDisabled`가 1이고, 충전 단계(`rechargeToFull`)로 전환되는 순간 0으로 내려간다 |미검증(캘리브레이션 완주에 수 시간) |
| 10 | **`push` 대 `applyRequested` 라우팅 (핵심 검증)** | 충전 한도 OFF + 발열 보호 OFF 상태에서 수동 방전을 시작한다(이 조합이면 일반 `push`가 "비활성화 요청"으로 오인하기 가장 쉬운 상태). 방전이 도는 동안: (a) 클램쉘 토글을 ON→OFF→ON으로 두 번 뒤집는다, (b) 외장 모니터를 뽑았다가 다시 꽂는다. 이 코드 경로는 SwiftUI `.onChange` 핸들러 본문이라 유닛 테스트로 도달할 수 없다 — 이 행이 유일한 안전망이다. | 매 조작 후 방전이 취소되지 않고 계속 진행 중이어야 한다(SoC 표시가 계속 움직인다). `SleepDisabled`만 1↔0으로 토글의 상태·모니터 연결 여부를 따라 바뀐다 |**통과** 2026-09-08. 한도 OFF+발열보호 OFF에서 토글 끔/켬/모니터 분리 3회 모두 방전 생존, `SleepDisabled`는 매번 20초 내 반영 |

### 실기 검증 요약 (2026-09-08, Mac17,2 / M5 / macOS 26.6)

10행 중 **7행 통과, 3행 미검증**. 핵심 검증 2건(1번 뚜껑 닫힘, 10번 라우팅) 모두 통과.
**실기에서 `SleepDisabled`가 갇힌 사례는 0건** — `kill -9`(4번), 어댑터 분리(2번),
모니터 분리(3번), 옵트인 해제(10번) 네 경로 모두 정상 해제됐다.

검증 중 드러난 것:

- **보조배터리로는 검증할 수 없다.** CHIE는 어댑터를 전기적으로 격리하지만 어댑터는 PD 협상
  주체로 계속 붙어 있어야 하는데, 보조배터리는 잔량이 줄면 출력 티어를 재협상하고 macOS가
  이를 어댑터 전환으로 인식해 방전을 취소한다. 20초 간격으로 취소되는 것을 관측했다.
  **벽 콘센트 + 정품 어댑터**가 필요하다.
- **4번 행의 기대문이 틀려 있었다.** "60초 내 reconcile로 다시 1로 복귀"라고 쓰여 있었으나,
  `persistPolicy`가 `manualDischargeActive = false`로 저장하므로 데몬만 재시작하면 수동 방전은
  설계상 재개되지 않는다. 방전이 없으면 `SleepDisabled`는 0으로 남는 것이 정답이다. 수정했다.
- 미러링이 켜진 외장 디스플레이에서도 판정이 깨지지 않았다(주 디스플레이가 외장이면 정상).
